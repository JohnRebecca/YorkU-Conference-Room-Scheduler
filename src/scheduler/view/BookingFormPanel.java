...
public class BookingFormPanel extends JPanel {
    ...
    private void refreshForCurrentUser() {
        ...
        roomRateLabel.setText("Hourly Rate: $" + user.getAccountType().getHourlyRate()
                + " / hour (" + user.getAccountType().getTypeName() + ")");
        ...
        idField.setText(user.getOrganizationIdOrStudentNumber());
    ...
    private JPanel createSchedulingCard() {
        ...
        JLabel idLabel = new JLabel("Organization ID / Student Number");
    ...
    private void confirmBooking() {
        ...
        String idValue = idField.getText() == null ? "" : idField.getText().trim();

        if (idValue.isEmpty()) {
            messagePanel.showError("A valid organization ID or student number is required to complete a booking.");
            return;
        }
        ...
        try {
            RegisteredUser user = userContext.getCurrentUser();
            user.setOrganizationIdOrStudentNumber(idValue);
