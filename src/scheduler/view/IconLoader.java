...
/**
 * Loads icons and room photos from a top-level "images" folder at runtime.
 * All lookups are optional: if a file is missing, the app keeps running
 * with no icon / a plain placeholder instead of crashing, so the UI works
 * before and after real image assets are dropped in.
 *
 * Expected folder layout (place "images" next to the folder you run the app from):
 *
 *   images/
...
public final class IconLoader {
    ...
    private static BufferedImage highQualityScale(Image source, int targetWidth, int targetHeight) {
        ...
        // Step down by halves until we're close to the target size, then do a final pass.
